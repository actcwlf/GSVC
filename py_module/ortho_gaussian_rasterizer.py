...
def ndc2Pix(v, rang_min, S):
    return ((v - rang_min) * S - 1.0) * 0.5
...
def in_frustum(orig_points, view_matrix, prefiltered, threshold=0.1):
    ...
    if p_view.z < 0 or p_view.z > threshold:  # // || ((p_proj.x < -1.3 || p_proj.x > 1.3 || p_proj.y < -1.3 || p_proj.y > 1.3)))
        if prefiltered:
...
def in_frustum_with_depth(orig_points, view_matrix, prefiltered, threshold=0.1):
    p_orig = float3(*orig_points)
...
def computeCov2D(
    ...
    limx = 0.65 * (x_max - x_min)
    limy = 0.65 * (y_max - y_min)
    center_x = (x_max + x_min) / 2
    center_y = (y_max + y_min) / 2
    t.x = min(limx, max(-limx, t.x - center_x)) + center_x
    t.y = min(limy, max(-limy, t.y - center_y)) + center_y
    ...
    # 低通滤波/卷积的结果就是 sigma + aI， a是滤波相关参数
    cov[0][0] += 0.3
    cov[1][1] += 0.3
...
def getRect(p, max_radius, grid):
    # ~~BLOCK代表的时tile数量~~ BLOCK似乎应该是tile大小
    rect_min = int2(
...
def ndc_culling(p_orig: float3, view_matrix, x_min, y_min, image_width, image_height, ndc_scale, radius=1):
    ...
    # point_image = float2(ndc2Pix(p_view.x, image_width), ndc2Pix(p_view.y, image_height))
    point_image = float2(ndc2Pix(p_view.x, x_min, ndc_scale), ndc2Pix(p_view.y, y_min, ndc_scale))
...
def rasterize_gaussians_filter(
    ...
    for idx in range(1000): # means3D.shape[0]):
        ...
        radius = 0
        if not in_frustum(orig_points, view_matrix, prefiltered):
