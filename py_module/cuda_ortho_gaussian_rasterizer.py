...
class _RasterizeGaussians(torch.autograd.Function):
    ...
    def forward(
        ...
        min_x, max_x = means3D[:, 0].min(), means3D[:, 0].max()
        min_y, max_y = means3D[:, 1].min(), means3D[:, 1].max()
        min_z, max_z = means3D[:, 2].min(), means3D[:, 2].max()

        # print(raster_settings.x_min, raster_settings.y_min)
        ...
        if raster_settings.debug:
            ...
            # cpu_args = cpu_deep_copy_tuple(args)  # Copy them before they can be corrupted
            # torch.save(cpu_args, "ortho_snapshot_fw.dump")
            num_rendered, color, radii, geomBuffer, binningBuffer, imgBuffer = _C.ortho_rasterize_gaussians(*args)
    ...
    def backward(ctx, grad_out_color, _, __):
        ...
        # print(cov3Ds_precomp.device)
        # print(radii.device)
        # print(sh.device)
        # print(geomBuffer.device)
        # print(binningBuffer.device)
        # print(imgBuffer.device)
