...
def generate_neural_gaussians(viewpoint_camera, pc : GaussianModel, visible_mask=None, is_training=False, step=0):
    ...
    if is_training:
        ...
        # in default setting, author performs anchor adjustment until iter 15_000
        if step > 10000:
...
def render(viewpoint_camera, pc : GaussianModel, pipe, bg_color : torch.Tensor, scaling_modifier = 1.0, visible_mask=None, retain_grad=False, step=0):
    ...
    if is_training:
        (xyz, color,
         ...
         bit_per_offsets_param) = generate_neural_gaussians(viewpoint_camera, pc, visible_mask, is_training=is_training, step=step)
    else:
...
def prefilter_voxel(viewpoint_camera, pc: GaussianModel, pipe, bg_color: torch.Tensor, scaling_modifier=1.0,
    ...
    print('start py prefilter')

    radii_pure_py = py_rasterizer.visible_filter(
        means3D=means3D,
        scales=scales[:, :3],
        rotations=rotations,
        cov3D_precomp=cov3D_precomp,  # None
    ...
    # with open('means3d_tensor.pkl', 'wb') as f:
    #     pickle.dump(means3D.cpu(), f)

    # for idx, (a, b) in enumerate(zip(tmp.cpu().long(), radii_pure_py.long())):
    #     print(idx, a, b)
    #     assert a.item() == b.item()
    # assert torch.allclose(tmp.cpu().float(), radii_pure_py.float())

    print('finish prefilter')
    # exit()
