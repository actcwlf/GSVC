...
class EntropyGaussian(nn.Module):
    def __init__(self, Q=1):
        super(EntropyGaussian, self).__init__()
        self.Q = Q
    def forward(self, x, mean, scale, Q=None, x_mean=None, quantized=False):
        if Q is None:
            Q = self.Q
        if use_clamp and not quantized:
            if x_mean is None:
                x_mean = x.mean()
            if not isinstance(Q, torch.Tensor):
                Q = torch.ones(1, device=x.device) * Q
            x_min = x_mean - 15_000 * Q.mean()
            x_max = x_mean + 15_000 * Q.mean()
            x = torch.clamp(x, min=x_min.detach(), max=x_max.detach())
        ...
        if not quantized:
            ...
            m1 = torch.distributions.normal.Normal(mean / Q, scale / Q)
            lower = m1.cdf(x - 0.5)
            upper = m1.cdf(x + 0.5)
        # likelihood = torch.abs(upper - lower)
...
class Low_bound(torch.autograd.Function):
    ...
    def forward(ctx, x):
        ...
        x = torch.clamp(x, min=Low_bound.min_val)
        return x
    ...
    def backward(ctx, g):
        x, = ctx.saved_tensors
        grad1 = g.clone()
        grad1[x < Low_bound.min_val] = 0
        pass_through_if = np.logical_or(
            x.cpu().numpy() >= Low_bound.min_val, g.cpu().numpy() < 0.0)
